...
exports.checkAndCreateWallet = async (someone) => {
  ...
  try {
    ...
    const day = 1 //today.getDate(); // gets day of the month (1 - 31)
...
exports.internalTransfer = catchAsync(async (req, res, next) => {
  ...
  if (req.body.eth > 0) {
    ...
    await myWallet.save({ validateBeforeSave: false });
    await recieverWallet.save({ validateBeforeSave: false });
  } else {
    if (req.body.weth > 0) {
...
exports.userToValidateNft = catchAsync(async (req, res, next) => {
  ...
  if (!soldNft) {
    return next(new AppError(`the users has not sold any artworks`));
  }
      ...
      const response = await fetch.default(
        `https://api.tatum.io/v3/ethereum/wallet/priv`,
        {
          ...
          headers: {
            ...
            "x-api-key": "8f77ec2e-d0ed-4c04-bf7e16abc",
          },
          body: JSON.stringify({
            index: wallet.derivationKey,
            mnemonic:
              "fun little blade notable  peace wish never year guess casual mail dial better spoon health side",
          }),
        }
      );
