...
exports.createNft = catchAsync(async (req, res, next) => {
  ...
  const TotalOwendNft = await Nft.find({ nftOwner: req.body.nftOwner });
  console.log("user total Nft:", TotalOwendNft.length, TotalOwendNft);
  if (TotalOwendNft.length >= 5) {
    const userWallet = await Wallet.findByIdAndUpdate(req.user.wallet.id, {
      mintFiveNft: false,
    });
  }
});
...
exports.getLandingPage = catchAsync(async (req, res, next) => {
  ...
  //   30
  // );
  const arts = await Nft.aggregate([
    { $match: { category: "arts", nftInMarket: true } },
    { $sample: { size: 30 } },
  ]);
  const gaming = await Nft.aggregate([
    { $match: { category: "gaming", nftInMarket: true } },
    { $sample: { size: 30 } },
  ]);

  const membership = await Nft.aggregate([
    { $match: { category: "membership", nftInMarket: true } },
    { $sample: { size: 30 } },
  ]);

  const photography = await Nft.aggregate([
    { $match: { category: "photography", nftInMarket: true } },
    { $sample: { size: 30 } },
  ]);

  const pfps = await Nft.aggregate([
    { $match: { category: "pfps", nftInMarket: true } },
    { $sample: { size: 30 } },
  ]);

  // const exhibition = await Exhibition.find({}).limit(30);
  const exhibition = await Exhibition.aggregate([{ $sample: { size: 30 } }]);
...
exports.pushNftToMarket = catchAsync(async (req, res, next) => {
  ...
  if (!req.user.role === "admin" && !(req.user.id === gottenNft.nftOwner.id)) {
    return next(
      new AppError(
...
exports.pullNftFromMarket = catchAsync(async (req, res, next) => {
  ...
  const gottenNft = await Nft.findById(req.params.id);
  if (!gottenNft) {
    console.log("No Nft with the id");
    return next(new AppError(`No Nft with the id ${req.params.id}`, 404));
  }
